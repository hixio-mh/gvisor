...
const IPv4MaximumOptionSize = IPv4MaximumHeaderSize - IPv4MinimumSize

// Each type of option should support these methods
...
// IPv4TimestampTime provides the current time as specified in RFC 792.
...
func (b IPv4OptionTimestamp) Pointer() uint8 {
	return b[IPv4OptionTimestampPointer]
}

// IncPointer increments the pointer field by the given size.
func (b IPv4OptionTimestamp) IncPointer(size uint8) uint8 {
	b[IPv4OptionTimestampPointer] += size
	return b[IPv4OptionTimestampPointer]
}

// Flags returns the flags field in the IP Timestamp option.
func (b IPv4OptionTimestamp) Flags() uint8 {
	return b[IPv4OptionTimestampOFLWAndFLG] & IPv4OptionTimestampFlagsMask
}

// Overflow returns the Overflow field in the IP Timestamp option.
func (b IPv4OptionTimestamp) Overflow() uint8 {
	return (b[IPv4OptionTimestampOFLWAndFLG] & IPv4OptionTimestampOverflowMask) >> 4
}

// IncOverflow increments the Overflow field in the IP Timestamp
// option. If it returns 0 it overflowed.
func (b IPv4OptionTimestamp) IncOverflow() uint8 {
	b[IPv4OptionTimestampOFLWAndFLG] += byte(1 << 4)
	return (b[IPv4OptionTimestampOFLWAndFLG] & IPv4OptionTimestampOverflowMask) >> 4
}
...
// ========================== options as a group ===============================
// This represents the option area in an IPv4 packet.
type IPv4OptionSet struct {
	buffer        []byte
	bytePointer   int
	optionPointer int
	size          int
...
// We create a new packet option descriptor  with this.
func NewEmptyOptions() IPv4OptionSet {
	options := IPv4OptionSet{
		buffer: make([]byte, 0, IPv4MaximumOptionSize),
		option: []IPv4Option{},
	}
	return options
}

// Copy a buffer of bytes into an OptionSet ready for parsing.
func CopyOptions(options []byte) IPv4OptionSet {
	l := len(options)
	if l > IPv4MaximumOptionSize { // XXX probbaly not in final code. caller should check
		panic(fmt.Sprintf("options too long, got = %d, want <= %d", l, IPv4MaximumOptionSize))
	}
	if (l & 0x3) != 0 { // XXX remove before review/submit
		panic(fmt.Sprintf("bad options length %d, should be a multiple of 4", l))
	}

	optionSet := IPv4OptionSet{
		buffer: make([]byte, l, l),
		option: []IPv4Option{},
	}
	copy(optionSet.buffer, options)
	return optionSet

}

// ===========  building up an option set from individual options ============
func (o *IPv4OptionSet) Push(option IPv4Option) int {
	n := copy(o.buffer[o.optionPointer:], option.Contents())
	return n
}
